...
进一步验证，我们自己创建一个`NSKVONotifying_BYPersonModel`类，添加 KVO 时，会发出警告
...
首先可以发现`setName:`方法的指针指向变了，从`[BYPersonModel setName:]`指向了 `Foundation `的 `_NSSetObjectValueAndNotify`的 C 语言方法
...
可以看到，`BYPersonModel`类被替换成了`BY_NSKVONotifying_BYPersonModel`类，也能监听到`name`的变化，手写 KVO 成功。
当然实际的 KVO 实现的细节远比我们手写的复杂，这个只是一探究竟而已。
